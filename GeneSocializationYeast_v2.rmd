---
title: "Gene Socialization in S. cerevisiae"
author: "Maria Kalyva, Ioannis Iliopoulos and Christoforos Nikolaou"
date: "January 2021"
output:
  html_document:
    df_print: paged
...
# Strands
genestrands<-read.delim("~/Dropbox/Data/SGD/7071genes_saccer2_leftandrigth_strands.bed", header=T, sep="\t")
...
###
fdata<-read.delim("MKalyva_Duplicated_genes2019_ACS.tsv", header=T, sep="\t")
...
#
tfconsdup<-read.delim("GeneDup_New_Analysis_TF_Cons.tsv", header=T, sep="\t")
...
#
synteny<-read.delim("YGOBSyntenyScores.tsv", header=T, sep="\t")
#
repTiming<-read.delim("~/Dropbox/Articles/Under_Prep/GeneSocializationYeast/sacCer2_RepTiming.bed", header=F, sep="\t")
colnames(repTiming)<-c("chrom", "start", "end", "Gene", "meanDNAReplicationTiming")
#
# Gene coordinates
...
# Tirosh-Barkai 43 genes with Assymetrical Transcription Correlation
# these are genes one of whose pair is correlated in expression with Candida but the other is not
divExpGenes<-c( "URA7", "YML125C", "YGR141W", "URA5", "SEC14", "YDR018C", "UTH1", "MRS3", "YDR341C", "SVL3", "YGL060W", "YKL035W", "FKS1", "APA1", "YOR054C", "EGD1", "PRK1", "YOR227W", "GSP1", "YBL054W", "EMP70", "YDR185C", "GZF3", "YEL006W", "ARE2", "YMR102C", "YIL036W", "YJL084C", "YOR108W", "YKR027W", "YHR149C", "YGL133W", "YER119C", "RSC6", "HXK2", "YLL010C", "AAP1'", "YGL144C", "PMT2", "NHP6A", "MKK2", "YBR238C", "CDC19") 
divExpGene_fdata<-c(30 , 847 , 670 , 289 , 138 , 609 , 816 , 666 , 158 , 5 , 110 , 12 , 722 , 63 , 668 , 266 , 475 , 267 , 238 , 157 , 728 , 429 , 74 , 987 , 855 , 503 , 289 , 984 , 35 , 251 , 338 , 658 , 447 , 764 , 110 , 63 , 107 , 536 , 756 , 728 , 367 , 923 , 662 , 519 , 49 , 190)
# Constanzo
...
# PhyloCSF
phyloCSF<-read.delim("~/Dropbox/Data/SGD/sacCer3_sgd_genes_TSS_plusmnin500_PhyloCSFpower.bwsum", header=F, sep="\t")
#which(is.na(rowMeans(phyloCSF[,5:104])))->x
#phyloCSF<-phyloCSF[-x,]
meanphyloCSF<-read.delim("~/Dropbox/Data/SGD/sacCer3_sgd_genes_phyloCSFpower.bwsum", header=F, sep="\t")
colnames(meanphyloCSF)<-c("chr", "start", "end", "Gene", "phyloCSF")
#
# Struc Conservation
...
# Co-expression Continents
continents<-read.delim("~/Dropbox/Teaching/theses/MGeorgoulopoulos/Data/Continents.tsv", header=T, sep="\t")
# Histone Modifications
hismodprom<-read.delim("~/Dropbox/Data/SGD/all_gene_profiles/all_genes_washuniv_hismod_normalized_promoter_regions.tsv", header=T, sep="\t")
hismodorf<-read.delim("~/Dropbox/Data/SGD/all_gene_profiles/all_genes_washuniv_hismod_normalized_ORF_regions.tsv", header=T, sep="\t")
# 3D coordinates
...
pdf("~/Dropbox/Articles/Under_Prep/GeneSocializationYeast/ArticleFigures/Figure1B_RelDistPlot.pdf", height=5, width=7);par(mar=c(5,5,5,3));plot(wgdssdreldist$reldist, filter(wgdssdreldist$fraction, rep(1/10,10)), type="o", pch=19, lwd=3, col="grey15", las=1, xlab="Relative SSD/WGD Cluster Distance", ylab="", main="Fraction of Relative Distances", cex.main=2, cex.lab=1.5, cex.axis=1.3);abline(h=0.02, lty=3, lwd=3, col="grey15");dev.off()
#
